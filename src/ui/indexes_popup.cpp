...
bool IndexesPopup::setup(std::vector<Index> indexes, std::function<void(std::vector<Index>)> setIndexesCallback) {
    m_indexes = indexes;
...
void IndexesPopup::onAdd(CCObject*) {
    m_indexes.push_back({ .userAdded = true, .enabled = true, });
    this->createList();
}

IndexesPopup* IndexesPopup::create(std::vector<Index> indexes, std::function<void(std::vector<Index>)> setIndexesCallback) {

...
bool IndexCell::init(
  ...
  CCSize const& size
    ...
    m_toggleButton = CCMenuItemToggler::createWithStandardSprites(
        this,
        menu_selector(IndexCell::onToggle),
        .6f
    );
    ...
    auto inputNode = TextInput::create(size.width - HORIZONTAL_PADDING*2 - m_buttonsSize, "Index url", "chatFont.fnt");
...
void IndexCell::updateUI() {
    m_toggleButton->toggle(m_index->enabled);
}

void IndexCell::onToggle(CCObject*) {
    m_index->enabled = !m_index->enabled;
    log::info("Index enabled: {}", m_index->enabled);
    this->updateUI();
    // Cancel the toggling of the button by cocos that happens after the callback.
    m_toggleButton->toggle(!m_toggleButton->isToggled());
}

void IndexCell::onDelete(CCObject*) {
  m_onDelete();
}

IndexCell* IndexCell::create(
  IndexesPopup* parentPopup,
  Index* index,
  std::function<void()> onDelete,
  CCSize const& size
